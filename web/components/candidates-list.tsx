 ...
 * /api/candidates (read-only, over the CSV import) on mount and offers a
 * per-workout Preview (dry-run) and gated Sync (live). Degrades quietly when
 * nothing is imported or nothing is pending.
 */
    ...
    <section className="mb-8">
      ...
      {phase === "loading" ? (
        <div className="rounded-lg border border-border bg-surface p-6 text-center text-sm text-text-muted">
          Checking for unsynced workouts…
        </div>
      ) : candidates.length === 0 ? (
        <div className="rounded-lg border border-border bg-surface p-6 text-center text-sm text-text-muted">
          {demo
            ? "The demo has no imported workouts, so there is nothing live to list here. The synced history below is sample data."
            : note
              ? note
              : "Every imported workout is already synced."}
        </div>
      ) : (
